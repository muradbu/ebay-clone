...
if(isset($_POST["submit"])){
    ...
    if(count($errors) == 1 && $errors["email"] == "Het opgegeven email bestaat al."){
    ...
    if($step == 2){
        if(isset($_POST["newpassword"]) && isset($_POST["repeatnewpassword"]) && isset($_POST["answer"])){
            $errors = ResetUserPasswordValidator::validate($_POST);
            if(!$errors){
                UserController::put($user["Username"],["Password" => $_POST["newpassword"]]);
